...
export default function SavedPostsPage() {
  return <RequireAuth>{() => <SavedPostsContent />}</RequireAuth>;
}
...
function SavedPostsContent() {
  ...
  useEffect(() => {
    apiFetch("/api/community/my-saved-posts")
  ...
  return (
    <div>
      ...
      <div className="mb-5 text-lg font-extrabold text-text">저장한 글</div>

      {loading ? (
        <div className="py-16 text-center text-text-faint">불러오는 중이에요...</div>
      ) : posts.length === 0 ? (
        <div className="py-16 text-center text-text-faint">아직 저장한 글이 없어요</div>
      ) : (
